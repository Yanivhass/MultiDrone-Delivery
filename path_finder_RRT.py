...
NUMNODES = 2500
RRT_AVG = 10
RADIUS = 40  # radius to end of line
IS_FLYING = 0
source = [70, 110]
goal = [550, 390]
flag = 0
...
# map_raw = mpimg.imread("/home/yakov/bebop_ws/src/my_project/scripts/map/map6.bmp")
# [width,height] = [len(map_raw[0]),len(map_raw[1])]
# x = 0

##Defenition of node class

class node(object):

    ##Initialization of a node

    def __init__(self,x_pos,y_pos,node_num):
        self.x_pos = x_pos
        self.y_pos = y_pos
        self.node_num = node_num

    # Turns a node into a point with x,y position
...
class path_finder():
    ...
    def controllers(self, position, angle, x1, y1, x2, y2):
        ...
        if right == 1 and left == 0:
            ...
            err_controller_value = round((self.kp_err * current_err + self.ki_err * self.err_integral + self.kd_err * self.err_derivative),
                                         4)
    ...
    def dist_line_to_point(self, start, end, point):
        if end[0] == start[0]:
            ...
            #    normalization_coeff=cv_image.shape[1] / 2#****
            # Distance between poine and  line formula
            current_err = (float(abs(-1 * rrt_curr_line_a * point[0] + point[1] - rrt_curr_line_b)) / (math.sqrt(rrt_curr_line_a ** 2 + 1)))
    ...
    def rrt(self):
        ...
        if (map.get_at(source)[0:3] == (0, 0, 0)) or (map.get_at(goal)[0:3] == (0, 0, 0)):
            ...
            for i in range(NUMNODES):
                ...
                while map.get_at((int(round(newnode.x_pos)),int(round(newnode.y_pos))))[0:3]!=(255,255,255):#if the newnode is on obstacle
                    rand = random.randint(0, YDIM - 1), random.randint(0, XDIM - 1)
                ...
                if self.dist(newnode.x_y_pos(),goal) <= EPSILON:
                    path_found = 1
                    path = find_path(graph, 1, node_name)  # type : object ##Finding shortest path from all the nodes of rrt algorithm using dijkstra
            ...
            if (path_found == 0):
                ...
                for i in range(len(path.nodes)-1):
                    pygame.draw.line(screen, red, nodes[path.nodes[i]-1].x_y_pos(), nodes[path.nodes[i+1]-1].x_y_pos())
                    azimuth = atan2(nodes[path.nodes[i+1]-1].y_pos-nodes[path.nodes[i]-1].y_pos,nodes[path.nodes[i+1]-1].x_pos-nodes[path.nodes[i]-1].x_pos)
                ...
                for j in range(len(path.nodes)):
                    if j % RRT_AVG == 0 and j != len(path.nodes):
                        pygame.draw.circle(screen, black, [int(nodes[path.nodes[j]-1].x_pos) , int(nodes[path.nodes[j]-1].y_pos)] , int(3))
                ...
                pygame.draw.circle(screen, black, [int(final_node_rrt_avg.x_pos), int(final_node_rrt_avg.y_pos)],int(3))
                ...
                for k in range(len(nodes_rrt_avg)-1):
                    rrt_avg_distances.append(round(self.dist(nodes_rrt_avg[k].x_y_pos(), nodes_rrt_avg[k+1].x_y_pos()), 2))
                    ...
                    pygame.draw.circle(screen, green, (int(nodes_rrt_avg[k+1].x_y_pos()[0]), int(nodes_rrt_avg[k+1].x_y_pos()[1])), RADIUS, 2)
                    pygame.display.update()
...
def main(args):
    rospy.init_node('path_finder_shapes', anonymous=True)
    path = path_finder()
    path.image_broadcast()
